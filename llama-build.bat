...
:: ── 1. Delete stale webUI artifacts ─────────────────────────────────────────
...
echo [llama-build] Stale artifacts cleared.
echo.

:: ── 2. npm run build (SvelteKit → tools/ui/dist/) ───────────────────────────
::   Output lands in tools\ui\dist\ which cmake priority-1 will pick up.
echo [llama-build] Building webUI (npm run build)...
pushd "%UI_SRC%"
call npm run build
...
if %NPM_RC% neq 0 (
    echo.
    echo [llama-build] npm build FAILED ^(exit %NPM_RC%^)
    exit /b %NPM_RC%
)
if not exist "%UI_SRC%\dist\index.html" (
    echo [llama-build] ERROR: tools\ui\dist\index.html missing after npm build.
    exit /b 1
)
