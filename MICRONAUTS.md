...
## N-gram evidence routing

Before any model is dispatched, `ngramRoute(text)` builds a 2-gram graph of the
incoming prompt and maps the dominant fold phase to a micronaut + system hint.
This makes routing **evidence-driven** rather than rule-driven.
...
prompt text
    ↓
buildNgramGraph(text, n=2)          — node.phaseName = hashPhase(ngram) % 6
    ↓
aggregate frequency per phase       — phaseWeight { Pop:12, Wo:8, Sek:31, … }
    ↓
dominant = argmax(phaseWeight)      — e.g. "Sek"
    ↓
PHASE_TO_MICRONAUT[dominant]        — { micronaut: 'coder', hint: 'Focus on precise code…' }
    ↓
system += '\n[Phase: Sek → coder] Focus on precise code generation and execution.'
    ↓
qwNativeAgentGenerate(routedSystem, …)
```

### Phase → micronaut mapping

| Phase | Micronaut   | System hint appended                              |
|-------|-------------|---------------------------------------------------|
| Pop   | `memory`    | Focus on retrieval, context, and references.      |
| Wo    | `khanary`   | Focus on planning and orchestration.              |
| Yax   | `librarian` | Focus on knowledge, explanation, and enumeration. |
| Sek   | `coder`     | Focus on precise code generation and execution.   |
| Chen  | `eliza`     | Focus on deep analysis and quality reasoning.     |
| Xul   | `ui`        | Focus on concise, structured output.              |

### Endpoints

| Endpoint | Method | Role |
|---|---|---|
| `/gen/agent` | POST | Agent loop — calls `ngramRoute()`, appends hint, runs Qwen + tools. Returns `{ route }`. |
| `/gen` | POST | Adviser/coder — `ngramRoute()` biases both step-1 planner and step-2 coder system prompts. |
| `/gen/ngram` | POST | Exposes the raw graph: `{ nodes, edges, stats }` with phase labels and G(Δθ) edge weights. |

### Geometric edge weights

Edges are weighted by `G(Δθ) = cos(Δθ_ij)` where Δθ is the phase separation in
the 6-point circle. Same-phase transitions (Δθ = 0) get weight 1.0; adjacent
(Δθ = π/3) get 0.5; opposite (Δθ = π) get −1.0 and are dropped at threshold = 0.
This means the graph is pre-filtered by fold geometry before any model is invoked —
cross-phase token paths are structurally suppressed, not learned.

### Implementation files

| File | Role |
|---|---|
| `dist/khanary-server/kuhul-server.cjs` | `ngramRoute()` + `buildNgramGraph()` + `/gen/ngram` + `/gen/agent` wiring |
| `desktop/FoldGeometry/NgramGraph.cs` | C# mirror — `NgramGraphBuilder.Build()`, `NgramGraphData`, `Walk()` |
| `desktop/FoldGeometry/FoldGeometry.cs` | `FoldBoundedAttention`, `A_ij = G(Δθ)·S(Q_i,K_j)`, `FoldOptimizer` |
