...
cbuffer AdamParams : register(b0) {
    ...
    float grad_clip;    // gradient clip bound: fixed 1.0 default, or adaptive (KuhulPhysics)
    uint2 pad;
};
...
void CSMain(uint3 gid : SV_GroupID, uint3 lid : SV_GroupThreadID) {
    ...
    // NaN -> 0 (direction lost), Inf/huge -> clamp to ±1 (sign preserved).
    // Prevents Inf grads from permanently corrupting the m/v moments below.
    if (isnan(g)) g = 0.0f;
    else          g = clamp(g, -grad_clip, grad_clip);
