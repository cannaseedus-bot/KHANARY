...
- **N-gram evidence routing** — `ngramRoute(text)` builds a 2-gram graph on the incoming prompt, assigns each n-gram a fold phase (deterministic hash mod 6), and finds the dominant phase by frequency. The dominant phase maps to a micronaut + a system-prompt hint that is prepended before model dispatch. Routing is evidence-driven (content of the text) rather than rule-driven (keyword matching). Edges are weighted by `G(Δθ) = cos(Δθ_ij)`, so cross-phase token transitions are geometrically suppressed before any model is invoked. Exposed at `POST /gen/ngram`; wired into `POST /gen/agent` and `POST /gen`.
- **Semantic MoE routing** — dominant phase → micronaut: Pop → memory, Wo → khanary, Yax → librarian, Sek → coder, Chen → eliza, Xul → ui. System hint is appended to the active model's prompt; W·C·R scoring on the GPU selects among admitted micronauts.
